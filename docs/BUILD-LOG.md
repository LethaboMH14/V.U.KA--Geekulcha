...
## 2026-09-16 SAST | Sibusiso Khumalo (via Cline assistant) | model: Claude Sonnet 4.5 | P2.3 discard-by-default ingest boundary — implemented + tested | ADR-0033 proposed

Changed: wired the P2.3/G20 embedding boundary into the server's ingest path. New `server/src/api/ingest.py` (`receive_sensor_embedding`) receives the transient sensor embedding in memory and compares it only against the consented enrolment set for the stated tenant and purpose — via `server/src/auth/governance.py`'s `retain_consented_match`, which until now had no production caller (finding R in the 16 Sep PR #27 review rework, branch `docs/ipeleng-pr10-review`). On a valid match the receipt the caller may persist is a match result plus consent reference plus retention bound — never the embedding, raw image or derivative; on a non-match it persists no biometric data by construction. Ambiguous matches raise rather than guessing a consent reference. Added `test/ingest_boundary_test.py` (nine tests) covering the three acceptance properties named in `docs/DISCARD-BY-DEFAULT-EMBEDDINGS.md` plus the G3 retention-bound check. Recorded ADR-0033 (Proposed, both-lead acceptance pending; numbered past ADR-0031/0032, which open PR #30 reserves). Swept `docs/CHECKLIST.md` P2.3 row, `docs/OPEN-GAPS.md` G20 status cell, `team/sibusiso.md` (current task, changed-this-session, running log, tool line).

Evidence: `python -m unittest discover -s test -p "ingest_boundary_test.py"` — 9 tests OK; `governance_contract_test.py` still 8/8 OK. Retention bound constant `EMBEDDING_MATCH_RETENTION_SECONDS` = 7 days, the conservative end of `docs/PLAN.md`'s recorded 7–30 day direction of travel (G21 Gated Access Areas Code posture); the test asserts the bound stays within that range and that the retained record carries `retention_expires_at`. The no-biometric-payload test scans both the persistence record and the evidence event for the embedding value in tuple and list form and for any `embedding` key.

Decision: boundary is code, not configuration — a future ingest implementation that accepts embeddings inherits the discard property from this call rather than from caller discipline. G20's architecture-decision half is discharged; s57(1)(a) prior authorisation (P2.4), subject access, deletion route and the final data-layer TTL stay open under G3. Comparison is tuple/list content equality on simulated embeddings; the numpy port note (`np.array_equal` or cosine with tolerance, per Lethabo's PR #27 note) is carried in the module docstring for the Task 4 port. Simulated `sim_subject_*` enrolments are rehearsal data; no real enrolment exists and none is asserted.

Needs/blockers: ADR-0033 needs Lethabo's acceptance (contract-adjacent decision, per docs/OVERLAPS.md both-lead rule for ADRs); the final TTL is the G3 data-layer decision. Numbering: if PR #30 merges first it keeps ADR-0031/0032 and this stays ADR-0033; if this merges first, PR #30 renumbers.
