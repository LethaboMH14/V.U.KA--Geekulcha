...
25 Sep follow-up, **PROPOSED pending review**: on `docs/khutso-p3-k2-main-restore`, Khutso's assistant restores PR #54's SAPS row to current `main`, records Khutso's explicit confirmation of the existing P3.K1 dispositions, and reconciles `docs/EVIDENCE.md`, `docs/CHECKLIST.md` and `team/khutso.md`. P3.K1 is ticked for the completed audit, with source flags preserved; P3.K2 stays unticked until this restoration is reviewed and merged. Sibusiso reviews first, then both leads. This supersedes only the 23 Sep claim's branch and checklist-status statement; it does not assert a new human decision or product capability.
