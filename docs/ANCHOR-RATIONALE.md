...
**Decided 17 Sep, ADR-0028:** no self-hosted calendar — that solves a problem this project doesn't have. The real mitigation is two ordinary client behaviours: **submit every root to multiple public calendars** (already the OTS client default), and **run `ots upgrade` promptly** once a submission's Bitcoin confirmation lands, after which that timestamp is fully self-verifying against Bitcoin alone — the calendar is no longer needed for verification at all. Both are `anchor/publish.py` logic (Task 5, not yet started), not an infrastructure commitment. **Do not claim either half is live until `anchor/publish.py` actually implements it** — until then, state plainly that roots are pending calendar aggregation, per the honesty ledger.
