...
**Business reasoning** — For a bank/insurer sale, the access model *is* the compliance story: a public hash makes the record verifiable by a stranger, while the evidence stays consent-gated. **Revised after Sibusiso's second-lead response (26 Sep):** the built server encrypts payloads with a **server-held** key (the escalation engine must read the duress outcome) and guardians hold **no copy** of the record, so "victim-held key" and "guardian blind backup" are recorded as **open design questions**, not settled controls. Only the parts the server implements (public-read anchor; payload-deletion erasure; no tokens; optional enterprise mirror) are proposed as decisions.
...
Changed: `context.md` (reconciled to the honesty ledger and ADR-0035; permissioned-write/public-read; tamper-evident/ECTA wording; POPIA s26 precision; "no tokens"; access model corrected to server-held key + no guardian copy; FSCA stated as our reading, not clearance); `pitch-deck-outline.md` (same compliance fixes; access model added and corrected; "5 pillars" marked draft-to-confirm); `docs/POTENTIAL-QUESTIONS.md` (access-model answer corrected); `docs/WAR-ROOM-PITCH.md` (war-room pitch and Q&A prep); `docs/reviews/ANCHOR-ACCESS-MODEL-ADR-PROPOSAL.md` (new — draft ADR extending ADR-0035 for Lethabo/Sibusiso/Ipeleng; narrowed to the built design after Sibusiso's response); this entry.
