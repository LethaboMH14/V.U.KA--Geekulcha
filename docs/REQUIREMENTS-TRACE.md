...
| V6 — duress PIN at any prompt raises a signed signal while the prompt looks normal | Vukosi; Sibusiso | T16 — settings prompt only; T09 — late check-in duress only | specified; acceptance pending | Other PIN prompts and the signed signal still lack direct §15 coverage; P3.K4/P3.K5 and server work remain open. |
...
| V8 — ordered encrypted queue, truthful delivery state and duress-safe phone timeline | Vukosi; Sibusiso | — | specified; test gap | T06–T09/T23 exercise retries, server escalation and timing, not encrypted local ordering, receipt-state display or suppression of incident details on the phone. |
...
| A5 — authenticated unpaginated genesis-to-head export with payloads, salts, proofs and receipts | Sibusiso; Ipeleng | T04 — tamper detection; T19 — subject auth | specified; acceptance pending | Completeness, order, unpaginated delivery and all required export fields still need direct evidence. Deletion is P1/P2, not A5. |
...
| Requirement | Owner | Direct §15 test and scope | Status | Evidence / gap |
|---|---|---|---|---|
| S1 — bank signal only on defined duress outcomes and with triggering outcome recorded | Sibusiso; Khutso (`sim_bank`) | T11 — send/no-send timing; T16 — settings duress | specified; acceptance pending | Trigger label in `bank_signal_sent` payload is not asserted by either test; P3.K5 remains open. |
...
The matrix covers all 32 §2 requirements. Seventeen rows have no direct §15 test for their defining behavior: V2, V3, V4, V8, V9, V10, G2, G4, G5, G6, A1, A4, A7, S4, P1, P2 and D2. The other 15 rows have a test for at least one clause; their evidence/gap cells identify clauses that still need verification. V1 is one example: T17 directly checks permission refusal, while explicit arming remains a test gap. No row is labelled passed. The work order's T01–T20 range is stale against the accepted T01–T24 table. Sibusiso and both leads must accept the owner assignments and gap disposition before P3.K3 is marked complete.
