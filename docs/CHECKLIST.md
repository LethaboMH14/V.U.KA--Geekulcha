...
| P2.2 | **POPIA position paper + ADR** — s26/s27 asymmetry (there is **no legitimate-interest ground for biometrics**), and whether **discard-by-default** removes the problem | Ipeleng | C2, C3 | Sep 17 | ☑ | Closed 16 Sep — `docs/POPIA-POSITION.md` + ADR-0031 via PR #30 (successor PR — #28 merged with the P2.1 commits only): s27(1) grounds quoted from the Act's full text (Accessible Law copy, ⚑ pending Gazette verification), ground-by-ground walk shows no basis for a non-consenting passer-by, s33 limb flagged circular-or-open, discard-by-default adopted; counsel Q1–Q5 |
