...
| P1.2 | Update industry size to **R87bn (2024)**, government segment **R7bn** | Khutso, Babatunde | C2 | Sep 15 | ☑ | Closed 16 Sep — verified present in `docs/SONKE-OVERVIEW.md`, `docs/COMPETITORS.md`; no stale industry-size figure found anywhere else including `docs/08-BUSINESS.md` |
| P1.3 | Correct the ADR count and retire "440 tests" (state **510 test-function definitions** with the command, or run the suites and record the collected count) | Khutso | C3 | Sep 19 | ☑ | Closed 16 Sep — **the "→25" instruction was itself stale**: docs/adr.md now genuinely has **32** ADRs (7 authored since the "25" figure was confirmed). `01-ARCHITECTURE.md`/`MASTER-CONTEXT.md` (current-state claims) corrected to 32; `LEAN-CANVAS.md`/`PLAIN-WORDS.md` (predecessor-lineage, "predates this hackathon") correctly kept 25. "440 tests" retired everywhere → 510 predecessor figure with caveat. New: this clean repo's own suite actually run and collected — **37 tests, 37 passing**, commands in `docs/EVIDENCE.md` |
...
| P2.3 | **Discard-by-default embeddings** — architecture, not configuration. Any embedding that does not match an enrolled, consenting resident is discarded | Sibusiso | C2 | Sep 18 | ◐ | Proposed boundary in `docs/DISCARD-BY-DEFAULT-EMBEDDINGS.md`; adopted as **ADR-0031** (16 Sep, PR #30); implementation test still pending |
