...
| P2.3 | **Discard-by-default embeddings** — architecture, not configuration. Any embedding that does not match an enrolled, consenting resident is discarded | Sibusiso | C2 | Sep 18 | ◐ | Unit-level discard and biometric-free evidence tests added; persistence-boundary acceptance remains open because `server/src/db/` has no layer |
