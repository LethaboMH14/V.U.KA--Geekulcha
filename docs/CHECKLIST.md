...
| P1.8 | Confirm the prize/deployment premise with `sonke@geekulcha.dev` — official page says *"there will be cash prizes"*, contradicting the 90-day-deployment assumption the strategy was anchored on. Tag `ASSUMPTION` until confirmed | Lethabo | — | Sep 16 | ◐ | 15 Sep: prizes confirmed `FACT` (Lead Facilitator email: cash prizes top 3 + Telkom support + international partnerships). Deployment premise still `ASSUMPTION` pending `sonke@geekulcha.dev` response |
...
| # | Item | Owner | Criterion | Due | State | Amended |
|---|---|---|---|---|---|---|
| P2.11 | ~~Fix the R1.30/month anchoring figure before it reaches a slide.~~ **CLOSED — see PR #7 (fix/sibusiso-p211-anchoring-cost), pending merge.** OpenTimestamps picked as primary (~R0/month), Hedera fallback ~R9–10/month at current price. Swept across every live doc. Superseded this branch's own `docs/ANCHOR-COST-RECONCILIATION.md`/`ANCHOR-COST-SWEEP.md`, which duplicated this work incompletely — deleted in favour of PR #7 | Babatunde, Sibusiso | Sep 17 | ☑ | Closed via PR #7 |

| P2.11 | ~~Fix the R1.30/month anchoring figure before it reaches a slide.~~ **CLOSED 17 September 2026.** Picked OpenTimestamps as the named primary chain (already the diagrammed default throughout `docs/01-ARCHITECTURE.md`) — corrected figure is **~R0/month** (public calendar servers), Hedera fallback recomputed at current price ($0.0008/message, repriced Jan 2026) ≈ **R9–10/month**. Swept across every live doc that stated the figure: `anchor/README.md`, `docs/00-SPEC.md`, `docs/01-ARCHITECTURE.md` (9 occurrences), `docs/08-BUSINESS.md`, `docs/HANDOVER.md`, `docs/LEAN-CANVAS.md`, `docs/SDLC.md` | Babatunde, Sibusiso | Sep 17 | ☑ | Closed 17 Sep, Sibusiso |
...
| P2.15 | ~~Decide and record: self-host an OpenTimestamps calendar?~~ **CLOSED — see PR #8 (fix/sibusiso-p215-ots-calendar-decision), pending merge, ADR-0028.** No self-hosted calendar; multi-calendar submission + scheduled `ots upgrade` in `anchor/publish.py` instead. Superseded this branch's own `docs/OTS-CALENDAR-DECISION.md` ("deferred" was not an actual decision) — deleted in favour of PR #8 | Sibusiso | Sep 18 | ☑ | Closed via PR #8 |
