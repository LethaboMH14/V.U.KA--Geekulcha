...
## ADR-0033: Embeddings are discard-by-default at ingest — only a match result plus its consent reference is persisted, and it is retention-bounded
**Status:** Proposed (2026-09-16) — both-lead acceptance pending
**Owner:** Sibusiso Khumalo (P2.3) — boundary proposed by Ipeleng in `docs/DISCARD-BY-DEFAULT-EMBEDDINGS.md` (G20), privacy analysis in the P2.2 POPIA position
**Context:** A face embedding is special personal information under POPIA regardless of storage format: s27(1) offers no legitimate-interest ground for special personal information, so "vectors not images" is not by itself a lawful basis. PR #10 shipped `retain_consented_match` with passing tests, but nothing in the ingest path called it — the discard-by-default boundary was demonstrated, not wired (`docs/reviews/IPELENG-PR10-REVIEW.md`, finding R, 16 Sep rework). The repository has no ingestion implementation yet (`server/src/api/` was scaffolding), so the boundary can be wired before a real ingest path exists to bypass it. `docs/PLAN.md` records the retention direction of travel as 7–30 days with auto-overwrite (agreed Gated Access Areas Code posture, G21); the final TTL is the G3 data-layer decision. `docs/PORT-DIVERGENCES.md` D3 and ADR-0030 reshaped the contract's `Sighting` to carry an optional `embedding_ref`, not the embedding itself — the wire already avoids shipping biometric payloads.
**Decision:** The ingest path applies the boundary in code, not configuration: `server/src/api/ingest.py` (`receive_sensor_embedding`) receives the transient sensor embedding in memory, compares it **only** against the consented enrolment set for the stated tenant and purpose (via `server/src/auth/governance.py`'s `retain_consented_match`, which becomes its production caller), and returns the receipt the caller may persist. On a valid match that receipt is a **match result plus consent reference plus retention bound** (`resident_id`, `consent_ref`, `tenant`, `purpose`, `observed_at`, `retention_expires_at`) — never the embedding, a raw image, or a recoverable derivative. On a non-match the receipt persists no biometric data by construction. The audit event records the decision shape only and carries no biometric payload. An ambiguous match (two enrolments with the same embedding under one tenant/purpose) raises rather than guessing a consent reference — a refusal is evidence, not an error to swallow. The retained record's retention bound starts at **7 days** (`EMBEDDING_MATCH_RETENTION_SECONDS`), the conservative end of the recorded 7–30 day direction of travel; widening it requires a recorded ADR. Acceptance evidence: `test/ingest_boundary_test.py` (nine tests) proves a non-match leaves nothing in the persistence boundary, a consented match retains the result with its consent reference, the audit record contains no biometric payload, cross-tenant/purpose enrolments do not unlock, an ambiguous match is refused, and the bound is within the recorded 7–30 day range.
**Consequences:** `retain_consented_match` is wired: the boundary is enforced where embeddings enter, so a future ingest implementation that accepts embeddings inherits the discard-by-default property from this call rather than from caller discipline. G20's architecture-decision half is discharged (privacy position + ADR + implementation evidence exist); the remaining POPIA gates — s57 prior authorisation (P2.4), subject-access path, deletion route, and the final data-layer TTL — stay open under G3 and are not discharged by this ADR. Comparison runs on tuple/list content equality with simulated embeddings; the port to real numpy vectors must switch to `np.array_equal` or a cosine-similarity threshold with tolerance (port note in the module docstring). The simulated `sim_subject_*` enrolments are rehearsal data; no real enrolment exists and none is asserted. Numbering note: ADR-0031 and ADR-0032 are reserved by open PR #30 (P2.2 POPIA ADR, P2.6 Gated Access Code design-floor decision); this ADR takes 0033 so the two branches do not collide — if PR #30 merges first this stays valid, and if this merges first, PR #30 renumbers.
