...
**Status:** Proposed (2026-09-25, tenth draft after nine adversarial review rounds). Proposed by Lethabo (co-lead, acting security lead). Direction chosen by Lethabo on 24 Sep: **the verifiable record is available off the phone only**. Binds when Sibusiso (second lead) accepts it, after Ipeleng's security review.
...
- §9: recovery (scheduled revocation), the pre-incident hold, guardian removal and deletion during an incident (queued, below);
...
4. **Recovery never silences the old phone.** Recovery is allowed (never blocked, and on the never-cut list).
   ...
   - **At `effective_at`.** A journey still armed on the retiring key is ended by the server with `journey_ended{reason: key_retired}`. That suppresses **only** the `contact_lost` that the key's retirement would itself cause. **A check-in already opened before `effective_at` keeps its full window:** its result from the retiring key is still accepted until that window closes, and if no result arrives its `no_answer` fallback fires as normal. An open incident stays open for guardians. After that the key's requests are rejected, and the old phone shows delivery truthfully as not sent (V8). Whoever holds it can then learn that a recovery happened, 72 h after it did.
   - **Recorded in the chain.** The `key_revoked` entry is written when the recovery is received and carries `revoked_key_id` and `effective_at`. The verifier accepts an entry from that key only if its `received_at` is before `effective_at`, with **one exception, applied identically at ingestion and in the verifier:** a `checkin_result` (and the `pin_authorised` it carries) is accepted after `effective_at` only if its `checkin_opened` was received before `effective_at` and the result was received before that check-in's fixed deadline. Nothing else about the key's authority is extended.
   ...
   - **The recovery code is not rotated in this design, by anyone.** Automatic rotation would let a coercer who recovers first lock the member out for good. Rotation from the phone would let a coercer holding the phone destroy the member's escape credential without ever learning it. Rotation waits for an **independent recovery path** (the safe-contact procedure). Recovery is rate-limited per code and network address, not per subject.
...
- Member rotation of the recovery code from the current device (eighth draft): a coercer holding the phone could invalidate the member's separately kept code.
- Suppressing `no_answer` at `effective_at` (eighth draft): it could cancel the fallback for a detection accepted just before the deadline.
...
- Tests (added to spec §15 on acceptance):
  ...
  - T67 (revised): every recovery schedules revocation for exactly 72 h after receipt. Safety events and duress authorisations from a retiring key are accepted until then; normal-PIN governance from it follows the duress path with a `no_effect` entry; a journey armed across `effective_at` ends with `key_retired` and no `contact_lost`; a check-in opened before `effective_at` still accepts its result, and still fires `no_answer` if none arrives, until its own window closes. Responses are identical in status, shape and timing for current and retiring keys, with and without an open incident. Guardians are notified of every recovery, and the old phone's journey list is byte-identical before and after it.
  ...
  - T69: a verifier that rebuilds the registry from the chain accepts an old-key entry received before `effective_at` and rejects one received after, except a `checkin_result` for a check-in opened before `effective_at` and received before its deadline, which it accepts. A result received after that deadline, or for a check-in opened after `effective_at`, is rejected. It shows a governance entry with a `no_effect` follow-up as attempted, not done.
...
- **Residuals:**
  ...
  - Whoever holds the recovery code controls recovery, and it can't be rotated until an independent recovery path exists. A coercer who forces it out can recover again after the member does. Guardians are told about every recovery, and the safe-contact procedure is the planned way out.
