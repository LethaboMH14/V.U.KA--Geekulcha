...
| **T30** | During an open incident, and for 6 h after its last PIN entry, the member-device export and My Record end at the pre-incident head for a normal and a duress authorisation alike (byte-identical responses), and the prefix verifies; export without a fresh `export` authorisation is rejected | Spec §9, §15 — ADR-0041 (THREAT-MODEL TM-C9) |
| **T47** | Attempts 1–3 each show the identical "Try again"; after the third, the outcome is `no_answer` at the deadline; a later normal PIN shows "Checked in" and doesn't stop it; a later duress PIN is a duress signal | Spec §8, §15 — ADR-0041 (THREAT-MODEL TM-C10) |
| **T50–T52** | Member-ended closure, the fallback deadline, the PIN-gated journey end | Spec §15 — ADR-0041 |

---

## 8 · Settled at the checkpoint (24 Sep 2026, ADR-0041)

These were the P3.L8 inputs. Lethabo decided each one on 24 Sep 2026, acting as security lead while Ipeleng is away. The **Status** column gives the decision; the binding text is in `docs/VUKA-2-SPEC.md` §8 and §9, and the reasoning in ADR-0041. The problem and options columns are kept as they were drafted.

| # | Item | The problem | The options on the table | Status |
|---|---|---|---|---|
| 1 | **G33 — normal-PIN outcome vs `contact_lost`** | A normal PIN alone never closes an incident (§5). So a false detection, then a normal PIN, then `journey_ended` or 90 s of silence fires `contact_lost`: guardians are alerted and the bank signal follows 3 minutes later. This contradicts `docs/STAGED-DURESS-DEFENCE.md` S5 ("check-in → normal PIN → closed") | (a) a normal PIN plus `journey_ended` closes the incident; (b) `contact_lost` never triggers the bank signal after a normal-PIN outcome | **Decided 24 Sep — option (a), with (b) as the backstop** (ADR-0041): a normal-PIN journey end closes an all-normal, never-escalated, duress-free incident; `contact_lost` after all-normal outcomes alerts guardians and never sends the bank signal. Spec §8; T50 |
| 2 | **G34 — a check-in that is never shown escalates nothing** | The `no_answer` deadline starts from `checkin_opened` (§8). If notifications are revoked or the check-in never renders, `opened` never arrives, so no deadline starts and nothing escalates (coercion catalogue I01, PR #50) | (a) server-side fallback deadline from `signal_detected` receipt; (b) keep §8 as is and accept the hole | **Decided 24 Sep — option (a)** (ADR-0041): fallback `no_answer` at `signal_detected` receipt + window + 30 s, cancelled by `opened`. Spec §8; T51 |
| 3 | **G35 — ending a journey is not PIN-gated** | An attacker holding the phone can end the journey; heartbeats then stop legitimately and `contact_lost` never fires (coercion catalogue F06, PR #50) | (a) PIN-gate ending a journey; (b) treat a journey end while a signal is open as a continuation, not a stop | **Decided 24 Sep — option (a)** (ADR-0041): ending a journey needs a PIN; a duress end looks identical and raises the full alarm. Spec §8, §9; T52 |
| 4 | **The recovery row and S4 export freeze** | The §9 recovery row (24 h post-recovery freeze on guardian changes and deletion, §3 above) and the S4 extension of that freeze to bulk export are proposed but unconfirmed | Confirm both as written, or amend | **Confirmed 24 Sep as written** (ADR-0041): the 24 h post-recovery freeze covers guardian changes, deletion and bulk export. Spec §9 |
| 5 | **T30 — export during an open incident** | `GET /v1/subjects/{id}/export` returns the full chain to the current head, so the export payload can contain `duress_pin` in the current `checkin_result` — visible to a coercer holding the phone (THREAT-MODEL TM-C9) | Export needs a fresh PIN authorisation for action `export`; while an incident is open, or under a duress authorisation, the export ends at the last head before the incident — a chain prefix still verifies, so the no-op stays convincing | **Decided 24 Sep as proposed** (ADR-0041), extended by row 7: the member device reads up to the pre-incident head. Spec §9; T30 |
| 6 | **T47 — wrong-PIN behaviour and attempt limit** | Nothing is specified: a coercer with the phone can guess the PIN without limit at the check-in (THREAT-MODEL TM-C10) | Shape to decide: attempt limit, lockout length and progression, and whether a lock event is visible to guardians | **Decided 24 Sep** (ADR-0041): identical "Try again" for attempts 1–3; from the third, `no_answer` at the deadline, any later entry shows "Checked in", a duress PIN still counts; no visible lockout. Spec §8; T47 |
| 7 | **6-hour identical-record hold after any PIN entry** | Named in tonight's review as needed; specified nowhere in the repo | Decide what "identical" covers (same action and target? the same record body?), what the hold does (hold the second record? return the first receipt? extend the earlier entry?), and which events it spans | **Decided 24 Sep** (ADR-0041): the *pre-incident hold* — while the incident is open and for 6 h after its last PIN entry, My Record and export on the member's phone end at the pre-incident head, identically for both PINs. Spec §9; T30 |
| 8 | **What a duress no-op shows once the delay has passed** | A duress removal "takes effect after 24 h" and a duress deletion "after 72 h" — both are hard no-ops (§3). Nothing says what the coercer sees when the promised deadline passes with nothing changed | (a) keep showing success indefinitely — correcting the lie while the phone may still be held endangers the victim; (b) a plausible "still processing" state | **Decided 24 Sep — option (a)** (ADR-0041): the phone keeps showing success indefinitely. Spec §9 |

The spec sections named in each row now carry the decided rule. Where §3–§6 above say an incident closes only on `stand_down` or the 6 h close, read them with the member-ended closure in spec §8 (ADR-0041).
