...
**Current task** — P3.A3 server hardening and anchoring on PR #51 (`feat/sibusiso-contract-v2`). Request/event signatures, actor-to-key binding, journey ownership, row-locked heads and encrypted payload storage passed local PostgreSQL tests. The exact 33-byte `0x02` key-manifest fingerprint was submitted to the existing pinned testnet topic at sequence 2 and independently read back. CI's PostgreSQL-backed test job and all six other jobs passed at head `8fea572` (run `36067039454`); detailed job logs were unavailable from the local GitHub CLI cache. Event-to-escalation wiring still awaits an accepted per-kind payload schema and receiver deduplication. The isolated Node sidecar still needs a batch coordinator and a real `0x01` root. Export refuses without fresh PIN authority; P3.A5 and deployment remain unbuilt. Lethabo/Ipeleng security review and live end-to-end checks remain outstanding. PR #51 remains open and is not deployed.
...
- **P3.A3 slice 2 / P3.A6 deployment:** slice 2 is implemented; security review remains pending. The PostgreSQL-backed CI tests and all other required checks pass at head `8fea572` (run `36067039454`). The server must remain unexposed until its security review and required slices are complete; do not deploy this branch. No key-manifest runtime is assumed by this slice.
- **P3.A3 slice 3 / P3.A5 export:** the incident-closure and export decisions are recorded in ADR-0041; implementation remains unbuilt. Event-to-escalation wiring is blocked on the accepted per-kind payload shape and receiver deduplication. Do not claim export is complete or deploy before required server work and review.
- **PR #51 sim_bank contract fixes (25 Sep, Codex acting at Sibusiso's request):** `contracts/openapi.yaml` now requires a non-empty `subject_id` documented for a `sim_` subject, requires the pinned-server signature scheme, and returns a receipt with `sim: true`; the trigger enum uses `duress_signal`. Journey end and export are labelled `INCOMPLETE` under ADR-0041. CI now runs a full sidecar `npm audit`. These are contract declarations only; sim_bank runtime authentication and ADR-0041 end/export enforcement remain unbuilt. Run `36067039454` passed all seven jobs at `8fea572`; Lethabo/Ipeleng review is still required, and PR #51 remains open with changes requested.
...
- 2026-09-25 — Codex assistant, acting at Sibusiso's request: completed the accepted PR #51 sim_bank contract fixes, renamed only the bank trigger value to `duress_signal`, marked ADR-0041 journey end/export operations incomplete, and removed the sidecar audit relaxation. Fresh isolated npm installs and the contract, shared, sidecar, Python and document checks are recorded in `docs/build-log/entries/2026-09-25-codex-sim-bank-contract-fixes.md`. The PR remains subject to Lethabo/Ipeleng review and fresh CI; runtime auth is not claimed.
