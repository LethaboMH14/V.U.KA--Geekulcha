...
**Current task** — P3.A3 server hardening and anchoring on PR #51 (`feat/sibusiso-contract-v2`). Request/event signatures, actor-to-key binding, journey ownership, row-locked heads and encrypted payload storage have passed local tests against real PostgreSQL. The exact 33-byte `0x02` key-manifest fingerprint was submitted to the existing pinned testnet topic at sequence 2 and independently read back. A generic durable outbox has local crash/reclaim tests; CI is being updated to run real PostgreSQL tests. Event-to-escalation wiring still awaits accepted per-kind payload schemas and receiver deduplication. The isolated Node sidecar still needs a batch coordinator and a real `0x01` root. Export refuses without fresh PIN authority; P3.A5 and deployment remain unbuilt. Lethabo/Ipeleng security review and live end-to-end checks remain outstanding. PR #51 is pushed, not merged or deployed.
