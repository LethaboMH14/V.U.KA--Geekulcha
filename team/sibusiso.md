...
**Current task** — P3.A3 server hardening and anchoring on `feat/sibusiso-contract-v2`. Request and event signatures, actor-to-key binding, journey ownership, row-locked heads and encrypted payload storage have local tests, including real PostgreSQL. The key manifest is merged and its exact 33-byte `0x02` fingerprint was submitted to the existing pinned testnet topic at sequence 2, then independently read back from the public mirror. The isolated Node sidecar still needs a durable batch coordinator and a real `0x01` root. Escalation/event wiring awaits accepted per-kind payload schemas; export refuses without fresh PIN authority, so P3.A5 and deployment remain unbuilt. Lethabo/Ipeleng security review and live end-to-end checks remain outstanding. This work is local only, not pushed or deployed.
