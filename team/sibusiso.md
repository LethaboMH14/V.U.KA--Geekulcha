...
**Current task** — P3.A3 server hardening and isolated Hedera transport on `feat/sibusiso-contract-v2`. Request and event signatures, actor-to-key binding, journey ownership, row-locked heads and encrypted payload storage have local tests, including a real PostgreSQL run. The key manifest is merged and the Node sidecar can form pinned 33-byte messages and require mirror read-back, but no typed message has been submitted to testnet: the original topic's private submit credentials are unavailable locally. Escalation/event wiring awaits accepted per-kind payload schemas; export refuses without fresh PIN authority, so P3.A5 and deployment remain unbuilt. Lethabo/Ipeleng security review and live end-to-end checks remain outstanding. This work is local only, not pushed or deployed.
