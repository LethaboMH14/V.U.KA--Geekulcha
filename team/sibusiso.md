...
**Current task** — P3.A3 server hardening and anchoring on `feat/sibusiso-contract-v2`. Request/event signatures, actor-to-key binding, journey ownership, row-locked heads and encrypted payload storage have local tests against real PostgreSQL. The exact 33-byte `0x02` key-manifest fingerprint was submitted to the existing pinned testnet topic at sequence 2 and independently read back. A generic durable outbox now has local crash/reclaim tests; event-to-escalation wiring still awaits accepted per-kind payload schemas and receiver deduplication. The isolated Node sidecar still needs a batch coordinator and a real `0x01` root. Export refuses without fresh PIN authority; P3.A5 and deployment remain unbuilt. Lethabo/Ipeleng security review and live end-to-end checks remain outstanding. This work is local only, not pushed or deployed.
