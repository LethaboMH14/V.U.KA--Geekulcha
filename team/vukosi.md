...
- 2026-09-26 — Codex assistant at Vukosi's request: P7b clarified report status as `computed_unverified` absent model/run/consent provenance, and suppresses M3 latency percentiles with `invalid_clock` whenever adjusted latency is negative while retaining the suspect count. Criterion C3; trust answer: a reviewer can distinguish computed synthetic output from evidenced measurements and invalid clock arithmetic cannot appear as a valid latency summary. Tests are synthetic/offline only; no measurement, model, audio, dataset, dependency, contract or threshold changed. Exact red/green and repository-check outputs are in `docs/workflows/vukosi/p7a-measurement-instrument/IMPLEMENTATION.md`. No approval or live evaluation is asserted.
