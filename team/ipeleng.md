...
**Current task** — Work order below (issued 23 Sep). Thu 24 done: PR #43 security review posted 23 Sep (APPROVE WITH CONDITIONS); PR #57 (ADR-0039) security review posted 24 Sep — APPROVE, the recorded security acceptance (conditions A–C in `docs/reviews/IPELENG-PR57-REVIEW.md`); P3.S3 owner review — `docs/security/THREAT-MODEL.md` ACCEPTED (63 rows, 55 mapped, 8 recorded observations) and `docs/security/TEST-SPECS.md` accepted with conditions (T04–T52; T24/T37 oracles close PR #43 B1) → PR #75; P3.L8 self-review + the PR #57 record → PR #76. Fri 25: key-manifest bootstrap, then **verify-min by 10:00** (hashes, prev links, device signatures, first broken index; T03/T04 green) for the 12:00 thin slice (RG1), then wire it to Sibusiso's server-min export. Every PR goes to **Lethabo for review**; nothing merges without his approval. The SSDLC is due Sat 12:30 (internal Sat 11:00).
...
- Current task / status: Work order issued 23 Sep 2026 (see the Work order section below); Fri 25 — verify-min by 10:00 for the 12:00 thin slice; SSDLC due Sat 26 Sep 12:30 (internal 11:00). P2.3/P2.6 parked with UMOJA/KHAYA. PRs #74/#75/#76 open — Lethabo reviews each; nothing merges without his approval.
- Claimed files / contract versions: none; reserve before editing.
- Last updated: 24 September 2026 — P3.S3 owner review done: `docs/security/THREAT-MODEL.md` ACCEPTED, `docs/security/TEST-SPECS.md` accepted with conditions (PR #75); P3.L8 self-review + PR #57 review record (PR #76). Fri 25: verify-min by 10:00; Lethabo reviews every PR.
...
- 2026-09-24 — Ipeleng (via Cline assistant): owner review of the security-test corpus done (P3.S3, evening). Accepted `docs/security/THREAT-MODEL.md` — status `PROPOSED` → `ACCEPTED`; 63 threat rows, 55 fully mapped to a spec ID and a test; the 8 unmapped (GD-4/5/6, VP-7, HD-5, BK-5, CI-1/2) stand as recorded observations with named owners, not new specs written tonight. Accepted `docs/security/TEST-SPECS.md` **with two conditions**: (1) where the spec says a field's behaviour is `UNDEFINED`, contract v2 decides it — writers do not invent values; (2) nothing is marked run from the spec document — a test is run only when its code runs. T04–T52 complete; T24/T37 oracles satisfy PR #43 condition B1. Decisions recorded: T49 stays 404 with a byte-identical body (existence must not leak); T47's `attempt` field accepted in principle, waits on contract v2 (#51). Hand-off sent to the writers: Sibusiso/Khutso take the server specs (T06–T11, T30, T34–T38, T40, T43–T45, T48, T49), Vukosi the device specs (T15–T18, T41, T42, T46, T47) — 20:00 deadline stands. Build-log entry `docs/build-log/entries/2026-09-24-ipeleng-p3s2-s3-owner-review.md`; checks pass (`check-threat-map.mjs` 63/55, `check-docs.mjs`); commit `a442bd4` on `docs/ipeleng-p3-s3-threat-model-test-specs` → **PR #75**; P3.L8 self-review, PR #57 record, build-log and this file → commit `8b24f67` on `docs/ipeleng-p3-l8-pin-authority` → **PR #76**. PRs #74/#75/#76 open and unmerged. Standing directive: **every PR goes to Lethabo for review; nothing merges without his approval.** Fri 25 plan: key-manifest bootstrap (`contracts/keys/manifest.json` — pinned Ed25519 server SPKI, topic id, network — plus validation vitest); **verify-min by 10:00** (hashes, prev links, device signatures, first broken index; T03/T04 green) for the 12:00 thin slice (RG1); wire it to Sibusiso's deployed server-min export and record the first roundtrip in the build log. Prerequisites flagged with their owners: Sibusiso's server-min deploy 09:00, Vukosi's T03 device-signature fixture, PR #74 merge (verify-min rebases after), key-manifest bootstrap before any signature check.
