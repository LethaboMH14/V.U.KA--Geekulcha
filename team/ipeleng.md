...
**Current task** — **Gated Access Areas Code review written 16 Sep** (`P2.6`, due Sep 17) — `docs/GATED-ACCESS-CODE-REVIEW.md` + ADR-0032: the load-bearing finding is that the draft Code recorded by `docs/PLAN.md` §C3 / G21 / P2.6 (published 30 Apr 2026, comments closed 14 May) **could not be corroborated on any source reachable this session** — the Regulator's site search returns zero hits, its codes-of-conduct page lists no 2026 entries (nearest verified artefact: the Residential Communities Council proposed code, received 08 Sep 2023), a web search for the exact title returns nothing — so every draft characterisation stays ⚑, and counsel Q1 asks for the Gazette notice via the s61(2) route or confirmation it does not exist. The verified spine (s60 scope levers reaching owners **and** their technology service providers; s61's ≤13-week process; s73(1)(c) making code breach "interference"; s14 minimal retention) anchors a gap analysis: the T0 ring buffer beats any retention bound, non-incident embeddings (30d) and location (7d) sit at/inside it, case-linked embeddings (case+90) and 12-month sightings exceed it **if** the scope reaches decision records (Q2 — kept as named parameters, not silently trimmed). ADR-0032 adopts the recorded direction of travel as a **binding design floor** (≤30-day auto-overwrite for gate-domain data, discard-by-default stands, FRT justification dossier before any pilot face gate) — safe under either outcome because every element is independently compelled (s14) or already accepted (ADR-0031/ADR-0002). Alongside, the WBS 1.1 verification record stands (`docs/audit/09`) and Information Officer registration stays **prepared, not submitted** (`P2.5` — leads' gate). Next: holder confirmations (closes `1.1`), then the **RICA position** (`P2.7`).
