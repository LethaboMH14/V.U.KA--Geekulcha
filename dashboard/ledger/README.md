...
**The VUKA system** (`#system`, linked as "VIGIL app" in the top bar) has one card per product. **VIGIL:** the demo Android APK from the `vigil-demo` release, an inline QR code for the same URL (dark on white in both themes, so cameras read it) and the plain line that there is no iPhone build yet. **ANCHOR:** `GET /healthz` read live with an 8-second timeout. If the browser can't read the reply (CORS, until `VUKA_DASHBOARD_ORIGINS` lists the Pages origin), it says "Status not readable from this browser yet", never "down". The card also links the OpenAPI contract and the server source. **Ledger:** this app. The footer's Products column, the app's footer and the Overview's "Azure API: ANCHOR on Home" link point back to it. `security.html` and `verify-min.html` carry a small site bar (Home, Ledger, Verify, Security).
