...
const blockedGate=JSON.parse(fs.readFileSync(path.join(fixture,'docs/security/intake-gate.json'),'utf8'));
blockedGate.status='blocked';
blockedGate.lead_approvals={lethabo:null,sibusiso:null};
fs.writeFileSync(path.join(fixture,'docs/security/intake-gate.json'),JSON.stringify(blockedGate,null,2)+'\n');
run(['add','server/example.py','docs/security/intake-gate.json'],0,'application fixture and blocked gate staged');
