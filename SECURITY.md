...
The workflow is authored locally; a green remote run is **not yet evidenced**. Sibusiso must push the reviewed branch, inspect the run and set main branch protection to require both jobs and the reviews in RULES.md. No CODEOWNERS GitHub handles are invented for members whose handles were not supplied. Verify both-lead review manually alongside required nonauthor approvals. Restrict workflow/config edits to reviewed PRs. Scanning configuration alone is not an enforced remote gate.
