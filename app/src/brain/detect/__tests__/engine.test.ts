...
function win(seq: number, label: string | null, bp: number, endMs = seq * 488): AudioWindow {
  ...
  return {seq, endMs, targetBp, topIndex: label ? TARGETS[idx(label)].index : 494, topBp: bp};
}
...
describe('class table', () => {
  it('matches the model label list, and fails closed on a moved label', () => {
    ...
    for (const t of TARGETS) labels[t.index] = t.label;
...
describe('impulses (glass, gun-like) confirm in one window', () => {
  it('fires on one glass window at or above threshold, with its reasons', () => {
    const {decisions} = run([audio(win(1, 'Shatter', R.thresholdBp.Shatter))]);
    const d = decisions[0];
    expect(d.fire).toBe(true);
    expect(d.candidate).toMatchObject({class_label: 'Shatter', class_index: 437, score_bp: 3000, threshold_bp: 3000});
    expect(d.reasons.map(r => r.rule)).toEqual(['argmax', 'threshold', 'confirm', 'motion']);
  });

  it('does not fire one basis point below threshold, and says why', () => {
    const {decisions} = run([audio(win(1, 'Gunshot, gunfire', R.thresholdBp['Gunshot, gunfire'] - 1))]);
    expect(decisions[0].fire).toBe(false);
    expect(decisions[0].reasons[1]).toEqual({rule: 'threshold', class_label: 'Gunshot, gunfire', score_bp: 3499, threshold_bp: 3500, pass: false});
  });
});

describe('voices confirm in 2 of the last 3 windows', () => {
  it('does not fire on a single scream window', () => {
    const {decisions} = run([audio(win(1, 'Screaming', 9000))]);
    expect(decisions[0].fire).toBe(false);
    expect(decisions[0].reasons[2]).toEqual({rule: 'confirm', family: 'voice', hits: 1, of: 3, required: 2, pass: false});
  });

  it('fires on the second hit within three windows, across voice classes', () => {
    const {decisions} = run([audio(win(1, 'Shout', 7000)), audio(win(2, null, 0)), audio(win(3, 'Screaming', 5000))]);
    expect(decisions.map(d => d.fire)).toEqual([false, false, true]);
    expect(decisions[2].candidate?.class_label).toBe('Screaming');
  });

  it('does not count a hit that has slid out of the window, or a skipped window', () => {
    // seq 1 and 4 are three apart: only one hit within the last three windows.
    const {decisions} = run([audio(win(1, 'Yell', 7000)), audio(win(4, 'Yell', 7000))]);
    expect(decisions.map(d => d.fire)).toEqual([false, false]);
  });
});

describe('argmax', () => {
  it('takes the highest target and ignores the rest of the window', () => {
    const w = win(1, 'Glass', 3600);
    (w.targetBp as number[])[idx('Screaming')] = 8000;
    const {decisions} = run([audio(w)]);
    // Screaming wins the window, so glass is not considered; one voice hit does not fire.
    expect(decisions[0].reasons[0]).toMatchObject({rule: 'argmax', class_label: 'Screaming'});
    expect(decisions[0].fire).toBe(false);
  });
  ...
  it('breaks a tie towards the lowest YAMNet index', () => {
    ...
    const {decisions} = run([audio(w)]);
    expect(decisions[0].candidate?.class_index).toBe(421);
  });
});

describe('cooldown and the open check-in', () => {
  it('suppresses a second detection within the cooldown, then allows it', () => {
    const t = R.thresholdBp.Glass;
    const {decisions} = run([
      audio(win(1, 'Glass', t, 1000)),
      audio(win(2, 'Glass', t, 1000 + R.cooldownMs - 1)),
      audio(win(3, 'Glass', t, 1000 + R.cooldownMs)),
    ]);
    expect(decisions.map(d => d.fire)).toEqual([true, false, true]);
    expect(decisions[1].reasons.at(-1)).toEqual({rule: 'cooldown', since_ms: R.cooldownMs - 1, cooldown_ms: R.cooldownMs, pass: false});
  });

  it('never fires while a check-in is open', () => {
    const {decisions} = run([{type: 'checkin', open: true}, audio(win(1, 'Glass', 9000))]);
    expect(decisions[0].fire).toBe(false);
    expect(decisions[0].reasons.at(-1)).toEqual({rule: 'checkin_open', pass: false});
  });
});
